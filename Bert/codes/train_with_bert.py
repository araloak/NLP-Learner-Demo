...
parser.add_argument('--pretrained_path', default="../pre_trained_models/chinese_L-12_H-768_A-12/", type=str, required=False, help='预训练模型保存目录')
...
def train():
    ...
    model.save("../models/"+str(args.times)+"/bert101112.h5")
def keep_train():

    model=build_bert(args)
    #model.load_weights(args.finetuned_path+"bert.h5")
    model.load_weights(args.finetuned_path+"bert.h5" , by_name = True, skip_mismatch = True)#只加载对应名存在的层
    data,label = get_finetune_data("../data/"+str(args.times)+"/train.tsv") #data是由所有句子组成的一级列表["你是人","你不是人"]
    ...
    model.save("../models/"+str(args.times)+"/bert_keep_train.h5")
def predict():
    model=build_bert(args)
    model.load_weights(args.finetuned_path+"bert.h5")
